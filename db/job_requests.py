...
def apply_jobs(user_id, job_id):
    try:
        ...
        cur.execute(f"UPDATE job SET applied_by = ? WHERE job_id = ?", (applied_by_str, job_id))
...
def withdraw_application(application_id: int):
    try:
        ...
        cur.execute(f"UPDATE applications SET application_status = 'withdrawn' WHERE id = ?", (application_id,))
        con.commit()

        # Remove the user ID from the applied_by column in the job table
        cur.execute(f"SELECT application_id FROM applications WHERE id = ?", (application_id,))
